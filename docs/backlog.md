...
> **⚠ SD-45 (2026-07-01): the BUILT M1 slice was CONDEMNED and REDESIGNED from scratch — see `docs/m1-redesign.md` + decisions.md SD-45.** The M1 *spec* (Part I shapes) stays canon; the SD-43/SD-44 presentation track below is HISTORICAL — do not resume it. The epics below remain valid as mechanic references (solver, SAT atom, faults, OSUB) but land inside the R0–R4 redesign phases, not on the old panel layer.
>
> **SD-45 BUILD STATE (2026-07-02, all committed to main; full detail per phase in decisions.md):**
> - [x] **R0 — sim substrate** (`299f00b`): antenna cards + T1/T2 bus tiers (capacity per-antenna, `NET_LINK_CAPACITY_UNITS` retired from the router); BEAMS (`net_assign_beam`, per-pipe fair-share/congestion; pointing = folded topology change); launch EVENT pipeline (countdown/ascent/deploy + seeded vehicle-loss/underburn/no-sep rolls, `net_circularize`); economy re-derivation (€40k start, stack costs, per-sat opex, 2× penalty, the no-contract-pays-its-own-provisioning theorem in `economy.test.ts`); act3a re-authored for the pipe model (BACKHAUL-3 shares the GEO BROADCAST pipe; the corridor = 3 pointed ACCESS LEOs); re-tame witness hardened (bandwidth-binding dip + player action after the dip + all-green split). ONE coordinated net-golden re-pin → `12975807967364188200n`.
> - [x] **R1 — the MISSION shell** (`070ddf8`, playtest-gated): net mode = ONE primary desktop (orrery hero + MISSION book⇄pad + WIRE + LEDGER·FLEET) + REVIEW; the pad = bus/cards/typed-orbit/comb/stack/ARM→LAUNCH; DEAD PRE-AIM (boot draft parks 90° W); beams cycle from fleet chips; `copy.ts` + `copy-lint.test.ts` enforce goals-never-instructions; briefing modals dead. Cold bot playtest: ≥5 real decisions, zero instructions, loop closes.
> - [x] **R2a–f — the hands + informed decisions** (`b4a329b`..HEAD): the FLICKER root-caused (de-squash surface-lift cliff under ULP rebase noise) + fixed structurally (ramped lift, by-construction disc placement, soft dither) and USER-CONFIRMED gone; DRAG-TO-AIM on the globe with damping + magnetic region snap + equator/pole inclination snaps (user-confirmed feel, "brutal" → assisted); the TWO-ROW COMB (fleet gaps vs fleet∪draft — aim replacements into holes); batch member park markers; wallet-vs-stack in red; floodlight/spot-beam rules-facts; per-beam "in view / NO LINE OF SIGHT"; parked-lon readouts; tooltips on every stat; beam pointer lines (red when blind) + honest launch arcs from ▲ CAPE CANAVERAL (launch site ≠ comms grounds — grounds stay equatorial/polar for act geometry, an R3 decision to revisit); ⌾ ground-station markers + labels; wheel zoom over the hero fill; queue rings on signed-and-dark regions; fresh-deploy pulses; MISSION hero framing 0.24 (GEO ring fits).
> - [~] **R3 — the hour + the economy**: RENEWALS live (completed terms re-offer grown demand at pay ×1.15 on a 30-min clock — the sustaining loop; `renewal.test.ts`); Act-2+ tenders carry OFFER WINDOWS with live "lapses in" readouts; acts 2–4 are PLAYABLE on MISSION (ROUTE SHORT/SPREAD toggle = the act3a relief; LAUNCH DEEP-SPACE RELAY = the act4 verb; fault WIRE lines + amber pulses live). **DONE (2026-08-08):** economy balance ✅ SD-51 (canon.ts extracted; hour closes +€3.5k, floor −€1.2k, terms cycle, renewals land in-session); fallback shortfalls on the MISSION book face ✅ (all four reworded lawful; copy-lint covers scenario.ts + bans API-name leaks); REVIEW gets THE PARSE in net mode ✅ (fold-derived run-at-rest; REVIEW preset mounts it); WIRE act-transition beats ✅; deploy pops ✅ (FL-14, were already live — per-member quadratic-decay flash). **REMAINING:** ground-station placement = ✅ SD-52 ACCEPTED fixed grounds, no verb; the M1-GATE human run (≥5 cold testers — the user's to run).

### FL — the verb rebuild (2026-08-07 audit consolidation; plan of record: `docs/first-light-integrated-plan.md`)

> The three verbs (launch / contract / loadout) rebuilt as instruments, then the style overhaul. Golden policy: exactly two net-golden re-pins (FL-01+FL-11 ✅ done; FL-07 planned).

- [x] **FL-01** loadout exploit fix: `DEFAULT_LOADOUT_CARD_IDS` priced before validate; always validate; PAD previews the effective loadout · net-golden re-pin #1 `71513456548811159n` (WITH FL-11) · `apply-action.test.ts`. (P1-C1)
...
>
> **PHASE 2c — DEPTH: Act-3 scarcity decision (SD-44, 2026-06-17).** Two surfaces for the §4.3 oversubscription call: a per-sat ALLOCATION LEDGER on LINK·LOAD (each sharing contract's served-share vs its floor — the fair-share math mirrored from router.ts — flagging who STARVES when over-cap) + a REROUTE PREVIEW on ROUTING·PREFER (current bridge + load, and where preferring bandwidth would move it). Pure per-frame reads; goldens untouched. KNOWN follow-up: the scenario doesn't reliably create a shared over-cap sat ("oversubscription never stressed"), so the ledger only fires when the player serves 2 regions off 1 sat — guaranteeing contention is a scenario/golden change, deferred.
>
> **PHASE 2d — TRIAGE: OVERVIEW status-board is a real glance (SD-44, 2026-06-17).** Added a network-health SUMMARY ("FLEET N sats · SERVED s/a · N AT RISK · +€/hr · wallet") + a per-demand at-risk REASON from the router's binding axis ("no path — launch over it" / "coverage gaps" / "too slow" / "over-cap — reroute"). Closes the loop with price-the-bet: accept an un-servable contract and the board shows the penalty bleeding (−€/hr, AT RISK). Pure reads (`fleet` + `NetContractRow.bindingReason`); goldens untouched.
...
### EPIC — Act 3c: The cislunar on-ramp (SD-62) · ✅ DONE · ↞ the rung the GDD asked for

> The arc used to step from Earth's ~3 ms straight to Mars's 8+ minutes. The GDD asks for cislunar FIRST in three places — the §2 tier table, milestone 3, and risk #7's mitigation (*"Teach light-delay by sight at cislunar's ~1.3 s before Mars bites"*). This closes that. The lesson is deliberately NOT a bigger number: the lunar farside is tidally locked away from Earth, so it is the first demand no Earth-orbit topology can EVER reach, and the answer is a node placed where neither endpoint is (Earth–Moon L2). See `docs/decisions.md` SD-62.

- [x] **M1-A3C-1** Cislunar geometry — *M* · new pure `net/cislunar.ts`: the tidally-locked lunar frame (built from the Earth→Moon direction each tick, so tidal lock is structural), the EML2 halo station, lunar occlusion, and the ~1.334 s ephemeris light-time. 20 tests assert the farside is invisible from Earth across a full lunar month AND from a wide shell of Earth orbital positions, that the nearside by contrast IS visible, and that the HALO is load-bearing (the bare L2 point cannot see Earth — the real Queqiao reason).
...
- [~] **M3 — Cislunar on-ramp:** Moon + L-points, first orbital datacenter, basic autonomy policies, observation contracts, light-delay teaching beat, DC thermal model. — *the **light-delay teaching beat** and the **Moon + L-point relay** halves landed early, inside M1, as Act 3c (SD-62): the GDD puts the cislunar on-ramp BEFORE Mars and M1's act4 Mars teaser was already shipping without it. The DC/autonomy/observation-contract halves remain M3.*
...
- [x] **X-04** Save/load robustness — JSON-serialisable from pure `src/sim/`; versioned saves + migration hook; fast snapshot load. **v1 (earlier):** the pure checkpoint envelope (`src/sim/net/persist.ts`) + the localStorage vault (`src/vault.ts`) + quick save/load (`v`/`V`) + a 120-sim-s autosave cadence, restore-is-bit-exact pinned by the fold hash. **X-04b (this pass):** the campaign now survives the TAB. A cold boot RESUMES the newest slot before the first sim tick, a `pagehide`/`visibilitychange` checkpoint closes the up-to-120-sim-s refresh window, the envelope carries the presentation-only accel/pause cursor (v2) with a real v1→v2 **migration ladder** (`migrateCheckpoint`; a save from the future or off the ladder reads as "no save", never a half-load), and the two deliberate cold starts land: `?fresh=1` (a scratch session — reads nothing, writes nothing, so it can't clobber the campaign it declined) and **NEW RUN** (shift-N: confirm → wipe → clean reload). Proven by `tools/scenes/resume.mjs` (17 assertions: build a run, hit F5, the same world comes back).
...
- [~] **RT-06** THE CONTENTION LEDGER + THE LOSS ROLL + RE-BEAM — *L* · pipe rows, segmented bar with the **Σfloor notch**, rider lines with STARVED/TIGHT flags, idle summary, the grouped loss roll (phrased causes, mm:ss, mean gap at ≥3), the REPOINT target picker (facts per option, `net_assign_beam`), the dark clock. **Removes the shortfall drain (main.ts:1366-1378) and halves the loss drain (1379-1390); updates `act1.mjs` + `hour.mjs` WIRE expectations in the same commit.** · Dep: RT-05. · Verify: full `node tools/playtest.mjs`; a starved rider at act3a; ≥3 stamps on one link with mean gap within 10 % of 150 s.
  > **PARTIAL (2026-08-19).** The contention ledger, the Σfloor notch, the rider flags, the idle summary, the grouped loss roll and the REPOINT TARGET PICKER all SHIPPED with RT-05/07 (one panel, one commit — splitting the render would have meant two DOM idioms). **Deliberately NOT done: removing the shortfall drain from SYSTEM.LOG.** TRACE is rail-summonable, not a desktop, so a player who never summons it would lose the §7.4 diagnosis entirely — that trades a §4.3 'not a log line' violation for a §5 'critical state behind a dig' one, which is worse. Revisit when/if RT-08 promotes TRACE to a desktop. The loss drain already de-duped to first-occurrence, so the 'halve it' half of the ticket was a no-op.
- [~] **RT-07** THE GLOBE COUPLING + FAULTS + AUDIO — *L* · traced-path brightening + hop pips, dimmed siblings, dashed unserved-with-path arcs, ghost-link on loss hover, candidate arcs, re-route flash + cell `← old`, the NODES sick group, three new cues (`link_lost`, `rider_starved`, `beam_committed`) + the health bed driven by max pipe utilisation. **Arc colour stays utilisation (`orrery-net-mode.test.ts` must stay green).** · Dep: RT-06. · Verify: `mono.mjs` per-distinction assertions; exactly one reroute cue per event.
  > **PARTIAL (2026-08-19).** SHIPPED: `tracedContractId` (the traced flow holds full strength, every other served path dims to 0.3× — the utilisation ramp is SCALED, never replaced, so `Orrery.utilColor` and its pinned test are untouched); dashed `--violet` candidate arcs region→sat for every other pipe whose link closes NOW; the re-route edge (`re-routed from NET-SAT-2 · GATEWAY` on the row for ~1.5 s); the NODES sick group; and the three cues `link_lost` / `rider_starved` / `beam_committed`, all edge-triggered. **Not done: hop pips along the traced path, the ghost-link on loss hover, and the health bed driven by pipe utilisation.** The first two need per-hop world points the orrery does not currently pool; the bed needs a second input to `setHealth`. None blocks the read — the traced path, the candidate absence and the re-route are the three things the judges called load-bearing.
