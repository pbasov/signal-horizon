# SIGNAL HORIZON
### Game Design Document — v0.8 (live document; git history tracks revisions)
...
**v0.8 (this revision) — naming the fun, and giving the escalation engine a real mechanism.** Two long-missing pieces, both foundational. First, **§3a states what the fun actually is**: the spine is *taming the sprawl* (chaos → order, visceral, measured against "does it work?"), and the mastery layer is *optimising against the parse* (order → optimal, cerebral, measured against "how close to the theoretical best?"). These are not two activities — they are **the same verb held to two different bars**, and the hinge between them is **a legible, honest record** (the "combat log for information delivery"). This is the concrete mechanism behind "easy to pick up, hard to master": a novice tames to *functional* and is happy; the optimiser lives in the parse and grinds the gap to *optimal*. Second, **§3b finally gives the escalation engine a mechanism** instead of asserting "bigger gap": three endogenous generators (demand grows where you serve · freshness decays so nothing stays solved · the leverage curve lets you automate the load) plus an across-tier rule — *each new tier must invalidate a strategy that worked the tier before*, or it's just a bigger number and should be cut. Escalation is what keeps chaos renewing **and** the optimum moving, so neither bar is ever permanently cleared. This promotes the optimisation layer (§4.2 multi-axis scoring, the §4.3a trace view) from footnote to spine via a new **§4.12 "The Legible Record (The Parse)"**, and sharpens the M1 gate to its truest form: *does the player finish a run wanting to look at what happened and do it better?* (§9). Pillar 6 is tightened to point at the mechanism rather than assert it.
...
6. **Success creates the next problem (the escalation engine).** Borrowed from OpenTTD: a profitable route invites traffic; traffic creates congestion; congestion forces you to re-engineer. Every win should visibly *strain* the network and generate the next gap to close. This is not a vibe — it has a stated mechanism (§3b): three endogenous generators (demand growth, freshness decay, the automation you build to keep up) that renew chaos *and* keep the optimum moving, plus an across-tier rule that each new tier must invalidate a prior strategy. The loop must escalate, not plateau — and it must do so *because of the player's own success*, not because the designer placed the next gap.
...
Before the loop's mechanics, the loop's *pleasure* — because the whole design is downstream of it, and naming it precisely disqualifies a lot of plausible-but-wrong instincts.

**The spine is taming the sprawl.** The most visceral fun this genre offers is *creating order from chaos* — a sprawling, straining system that was beating you comes under your control. This is the sysadmin/devops pleasure (one server by hand → ten with Ansible → a thousand with Kubernetes): the complexity never shrinks, your *altitude over it* rises. Crucially, **you can only tame complexity you can see and comprehend** — incomprehensible complexity isn't tamable, it's just suffering (the Aurora-4X failure mode). This is why §5 (visualization) is the make-or-break pillar: it is not UX polish *in service of* the fun, it *is the delivery mechanism for* the fun. Taming is measured against the bar of **functional** — did it become order? Does it work? Binary-ish, gut-level, immediate. This is the floor, and anyone can feel it.

**The mastery layer is optimising against the parse.** Once a system *works*, a second, deeper pleasure opens: grinding it toward *optimal*. This is the WoW-combat-log pleasure (analysing the parse afterwards — where did damage leak, which cooldown misaligned to which phase) and the Zachtronics pleasure (you solved it; here's the histogram of your solution's cost/cycles/area against everyone else's; now make it *better*). Optimising is measured against the bar of **optimal** — given that it works, how close is it to the theoretical best? Continuous, cerebral, and *deferred*: the fun is found in the record, between runs, in the gap between your 73% and the achievable 91%.

**They are the same verb at two bars — and the hinge is the legible record.** Taming-to-functional and taming-to-optimal are not two activities; they are one gesture (get the system under control) held to two standards. What converts the first into the second is **an honest, complete, legible record of what happened** — the "combat log for information delivery" specified in §4.12. A novice never opens it and tames happily to functional; the optimiser lives in it and grinds toward optimal. This is the concrete mechanism behind *easy to pick up, hard to master*: not a casual game with a hardcore mode bolted on, but one continuous staircase where the optimisation is always one glance at the record away the moment a player wonders "could that have been better?"

**A design constraint that falls out of this:** the floor verb and the ceiling verb must be *the same verb*. Laying one link (floor) and architecting a cycle-robust laser backbone (ceiling) have to feel like the same activity at different altitudes — *laying a link, but now thinking in topologies* — or the game is a casual toy with a separate expert game stapled on. The test, applied to every system: is the master's version of this just the novice's version held to a higher bar, or is it a different thing the novice never touches? If the latter, redesign until it's the former.

**The honest framing of the optimal.** Signal Horizon's systems are *honest* — real light-delay, real link budgets, real orbital geometry, real DTN. So the knowledge a player gains by optimising is *transferable*: figuring out "pre-stage the cache before the predictable conjunction blackout and you beat the light-gap" is a real insight about how delayed networks behave, true outside the game too. That is the rarest and most durable pleasure a sim can offer — comprehension crossing into exploitation — and it is only possible *because* the sim is the truth layer (§4.1). The design should lavish its attention on the mechanics where the optimal play is a *real insight a player can discover* (conjunction pre-staging, edge-compute-vs-ship-raw, latency arbitrage), not a number the game hands them — those are the moments worth the most, because they stay true when the player closes the game.

### 3b. The Escalation Engine — Why the Next Gap Exists

Pillar 6 promises "success creates the next problem." This section says *how* — because there are two ways to generate "the next gap," and only one of them works.

**The weak way (rejected): the designer places it.** You reach Mars; the tier table hands you the Belt; the Belt is "bigger" because the latency number is bigger. This is content-tiering, and it produces exactly the same-y feeling that kills the genre — the Belt is just Mars with a bigger number, the same game replayed at higher figures. If escalation is *only* this, the loop doesn't escalate, it *repeats.*

**The strong way (the design): your own success endogenously degrades your position.** The next gap is *self-generated* — your solution to the last problem becomes the cause of the next, the way automating green circuits in Factorio starves your copper. Three generators, all endogenous:

1. **Demand grows where you serve (the primary generator).** Serve a region well → its economic weight grows (the §4.9 network effect) → it now demands more than the capacity you built for it. *The gap you closed widened itself by being closed.* This is the OpenTTD loop, and it is the heartbeat: every served gap re-opens one size larger.
2. **Freshness decays, so nothing stays solved (the treadmill).** Unique to this game's physics: a Factorio belt, once built, stays built — but a freshness contract is *never* done. Stop feeding it and it decays (§8, §4.10). So "the next gap" is often not spatial-and-bigger but *temporal-and-recurring* — the same Mars feed going stale again because demand rose and your prefetch didn't keep up. A *different texture* of gap at the *same* location, which is itself an antidote to same-y.
3. **The automation you build to keep up (the leverage curve, §4.11).** Generators 1 and 2 create a rising load; the response is to *automate* it — and "how do I stay lazy and make the system run itself" is the purest devops pleasure. But the automation you build is sized for *today's* scale, and generator 1 guarantees tomorrow's scale outgrows it. So you build a higher-altitude tool, which is itself outgrown later. This is the renewable taming cycle: **tame → outgrow → re-tame higher.**

These three are not alternatives — they are **a single three-stroke engine**: growth raises the load, decay means the load is continuous (never a one-time build), and automation is you taming the load until growth breaks your automation's scale. Allocation tension (sell-the-pipe-vs-feed-the-mint, §4.10) is the *texture* of the hard choices inside the loop.

**Escalation also keeps the optimum moving — which is what makes optimisation (§3a) renewable.** A *static* system gets fully optimised and then is solved and dead (which is why WoW resets the parse every raid tier). Because demand grows, the optimum you were grinding toward *shifts*: your provably-near-optimal network is suddenly suboptimal again, a fresh gap between actual and optimal opens, and you re-optimise. So the same engine feeds both bars from §3a — it renews the *chaos* the tamer fights and moves the *optimum* the optimiser chases.

**The across-tier rule (the same-y defense, made into a hard test).** Within a tier, escalation is endogenous (the three generators). *Across* tiers, escalation must be a **new dominant constraint, not a bigger number** (Pillar 5) — and the hard test is: **does reaching the new tier make a strategy you relied on the previous tier stop working?** The Belt isn't "Mars but farther" — it's where round-trip coherence (viable to Mars) becomes physically impossible, *forcing* pure store-and-forward and predictive replication: a different game, not a scaled one. Jupiter isn't "the Belt but farther" — it's where radiation and one-way DTN break assumptions that held at the Belt. **If a proposed tier doesn't invalidate a prior strategy, it's just a bigger number and it should be cut.**

**The escalation engine.** Crucially, success doesn't end the loop — it *strains* it. Serving a region grows its demand; a profitable link congests; a winning cache stales faster as the world it describes moves; the automation you built is outgrown. Every solved gap births the next one — *because of your success, not because the designer placed it* — and that is what produces the "just one more fix" compulsion, and the emergent "and then this happened" story (§3) with no authored content required.
...
**Multi-axis scoring (the optimisation spine, not a footnote).** These dimensions are not just internal state — they are how the player's network is *scored, recorded, and grinds toward optimal* (§3a). A solution is rated on competing axes (coverage / latency / freshness / cost), so there is no single "right" build, just elegant trade-offs — which is what gives the optimisation layer somewhere to go. The data these axes produce is the substance of the legible record / parse (§4.12): the thing the optimiser analyses between runs to find the gap between actual and achievable. Late-game, the game renders a player's network as a shareable **"network portrait"** with its stat histogram against other players' builds (the Zachtronics "here's your solution vs everyone else's" hook — which, if §3a is right that optimisation is a core fun, is *spine*, not re-engagement garnish).
...
### 4.12 The Legible Record (The Parse) — The Floor/Ceiling Hinge

§3a established that the mastery fun is *optimising against a record*, and that the record is the hinge converting "make it work" (floor) into "make it optimal" (ceiling). This section specifies that record. It is the game's **combat log for information delivery** — the WoW parse, the Zachtronics histogram, made native to a network sim.

**What it must be (the preconditions, from §3a):**
- **Complete and honest.** Every served/missed/stale contract, every cache hit/miss, every prefetch (timely or wasted), every link drop and re-route, every blackout — timestamped, truthful. This is *cheap* here because the sim *is* the truth layer (§4.1): the record is just the sim's own event stream, surfaced. A record that lied or hid things would make analysis worthless (and break the transferable-knowledge pleasure).
- **Measured against an achievable optimum.** The fun is the *gap* between what you did and what was possible. So the record doesn't just say "73% freshness" — it says "73%; the achievable ceiling on your topology was ~91%; you missed the pre-conjunction prefetch window by 4 hours." The solver that routes the network (§4.3) can also compute "what was the best achievable here," giving the parse a *bar to measure against* rather than a bare number.
...
- **Per-contract post-mortem** — the damage-meter line item: delivered freshness/bandwidth vs. the contract's demand vs. the achievable optimum, with the specific miss called out ("prefetch fired late," "cache evicted the wrong dataset," "routed over a congesting link").
- **The freshness/utilisation timeline** — a replayable, scrubbable trace of every feed's freshness and every link's load across the session: *where* it decayed, *when* a cache went stale, *which* prefetch was mistimed. The combat-log scrubber.
- **Efficiency-vs-theoretical** — delivered value per € (and per bit) against the optimum the solver proves was achievable on the network you actually built. The single headline number the optimiser grinds down.
- **The trace view (§4.3a) is the live face of this** — "show me this flow's path right now" is not only live theatre, it is *the analysis instrument*: the `mtr` and the log-scrubber are the same tool, used live or post-hoc. Pick a flow, see where the latency and staleness accumulate along its real path, find the leak.

**Relationship to the floor.** The novice never opens the parse and is not punished for it — they tame to functional via the *live* cues (freshness-as-saturation in §8 is the floor's at-a-glance hint of "you're leaving freshness on the table"), and they have a complete game. The parse is dark matter for them and the whole game for the optimiser. That asymmetry *is* "easy to pick up, hard to master" — and it is why the parse must be present from the first playable economy (it underlies M1), even though most of its depth is invisible to most players.

**Scope honesty.** The *live* cues (saturation, the trace view, the status strip) are M1-era and cheap. The *full* post-run parse with achievable-optimum computation is an M2+ build (it needs the solver mature enough to compute "best achievable") — but the M1 economy must *log truthfully from day one* so the parse has real data to surface later, and so the M1 gate can ask its sharpest question (§9): *did the player finish wanting to do it better?*
...
9. **The Parse (the legible record, §4.12):** the post-run / at-rest analysis view — per-contract post-mortems, the scrubbable freshness/utilisation timeline, efficiency-vs-theoretical. Dark matter for the novice, the whole game for the optimiser. The trace view (4 above) is its live face; this is its reviewable-at-rest face.
...
**Milestone 1 — The irreducible fun test (prove the *design* hard part):** Earth-orbit + a relay to Mars, **honest light-delay, and the caching/prefetch decision loop**, with one glanceable map (the growing web + packets), one finance panel, and a truthful event log underneath (the M1-era seed of the parse, §4.12). Multi-axis scoring stub. **The question this milestone answers, sharpened by §3a:** the loop must contain at least one full *strain → relief* arc (feeds going stale faster than you can hand-manage → you discover the lever that tames it → control), and the truest gate is **"does the player finish a 30-minute run wanting to look at what happened and do it *better*?"** A tester who finishes satisfied-and-done built a toy; a tester who immediately wants another run to fix the thing they now see they did wrong is the taming-then-optimising fun (§3a) firing. *(Note: this means the M1 scenario must be just past the threshold where manual management strains — not one relay you babysit, but enough simultaneous feeds that you feel it getting away from you, so the tame-it lever has something to tame.)* If it passes, there is a game. If not, no downstream content saves it, and we pivot the visualization/core before building anything else. *This milestone gates the entire project.*
...
**Explicitly deferred / cut from the v1 critical path:** multiplayer (separate project; affordance only, §7); the mobile companion (no version target, §6); the optional local LLM (cut, may never ship, §4.6); the outer-system and interstellar tiers (post-1.0); most of the nine dashboards (one excellent view per milestone, §5).
...
*End of v0.8. This remains a living document (git history tracks revisions); Milestone 0 + the M1 fun-test gate everything else and should begin before any further system design is locked. v0.2 added datacenters and the autonomy/AI layer and locked the 1-bit art direction. v0.3 added the information-economy endgame and reframed mobile as a companion. v0.4 was the research-driven revision (visualization as the make-or-break pillar; currency flip made optional/gradual/€-preserving; brokering constrained; datacenters fenced; LLM cut; waiting-as-gameplay; escalation engine; roadmap resequenced). v0.5 reintroduced colour under one rule — *monochrome machine, living signal*. v0.6 made network topology a designed system (§4.3a). v0.7 added the 7th pillar "Leverage compounds" (§4.11) and recorded the stack move to TypeScript + Three.js + Vite, browser-native. **v0.8 names the fun and gives escalation a mechanism: §3a states the spine (tame the sprawl, measured against *functional* — the visceral floor) and the mastery layer (optimise against the parse, measured against *optimal* — the cerebral ceiling), the two being the same verb at two bars hinged on a legible record; §3b replaces "bigger gap" hand-waving with a real three-stroke engine (demand growth + freshness decay + the automation you build to keep up) that renews chaos and keeps the optimum moving, plus the across-tier "must invalidate a prior strategy" hard test; §4.12 specifies the parse (the combat-log for information delivery) as the floor/ceiling hinge and promotes the optimisation layer from footnote to spine; and the M1 gate is sharpened to "does the player finish wanting to do it better?"** The through-line of every change since v0.3: the genre's best games (Factorio, DSP, OpenTTD, Mini Metro, KSP, EVE, Zachtronics) win by making a system *visible, escalating, legible, and worth optimising* — and the single greatest threat to Signal Horizon is that its product is invisible. Everything here bends toward fixing that.*
