...
> **⚠ SUPERSEDED (2026-05-30).** This isolated-Act-1 prototype plan is **superseded by `docs/signal-horizon-m1-design.md`** (M1 AS ONE GAME). The new design builds the real, generalizing connectivity/coverage + launch-planner + routing-solver + fault systems under a state-gated arrival sequence, with Act 1 as the first slice on them. It **drops** every contraption this doc carried: the bent-pipe reachability stub, the co-located ground, the `A1_DISH_EIRP=1.1` EIRP-clip closing lever, the `windowCoverage.minFraction=0.802` / `A1_AVAILABILITY_BAR=0.85` forced imperfection, and the throwaway golden. **KEPT:** the spinning-Earth frame, the toy-radius GEO-parks pacing, the unforked m2 orbit propagation, radians-at-the-boundary. Read this doc only for the verified physics notes; the build plan is the new doc.
>
> **Status of this revision (historical).** This is the FINAL, build-ready Act-1 design. It supersedes the prior no-rotation design. The user's governing decision — **add a spinning Earth so a GEO satellite genuinely PARKS over a fixed spot** (spec §2: "GEO parks over a spot; LEO sweeps") — REPLACES the prior Decision A (the no-rotation "dwell game" workaround) entirely. Everything else still sound (isolation discipline, new-golden strategy, radians-at-the-boundary, planner-assist seam, window-coverage preview) is kept.
>
