...
export class LinkLoad implements PanelHandle {
  ...
  private rows = new Map<string, { root: HTMLElement; head: HTMLElement; bar: HTMLElement; fill: HTMLElement; pct: HTMLElement; meta: HTMLElement }>();
  private sig = "";
  ...
  render(state: LinkLoadState): void {
    ...
    if (sig !== this.sig) {
      ...
      for (const r of state.rows) {
        ...
        root.append(head, bar, meta);
        this.rowsHost.append(root);
        this.rows.set(r.satId, { root, head, bar, fill, pct, meta });
      }
    }
