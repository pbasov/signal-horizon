...
import {
  ...
  NET_OFFER_WINDOW_S,
  ACT3B_FAULT_SCRIPTS,
  // SD-56 — the console's mission browser fires a beat's OWN emit, never a hand-copied offer.
  M1_SCENARIO,
...
window.addEventListener("keydown", (e) => {
  ...
  if (netMode) {
    if (k === "1" || k === "2") setWmPreset(Number(k) - 1);
      ...
      // SD-56 — THE ONE SWITCH (shift-\): unlock every act, put every authored mission on the
