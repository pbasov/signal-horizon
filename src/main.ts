...
// SD-70 — THE DEV CONSOLE: the dev-only cheats/debug tile (skip acts, warp time, top up the
...
// ── SD-70 — THE DEV CONSOLE (cheats + debug, a tile of its own) ────────────────────────
...
if (netMode) {
  ...
  // SD-70 — THE DEV CONSOLE, dev builds only (or ?dev=1). Registered like any other host so it
...
// SD-70 — the DEV button is APPENDED to the rail set only when the console is enabled, so a
...
function frame(now: number): void {
  ...
  // SD-70 — the DEV CONSOLE's standing locks + the TURBO drain (extra ticks past the clock's
  ...
  if (netMode) {
    ...
    // SD-70 — the DEV CONSOLE repaints only while it is actually on screen: its read takes a
